...
def move_daphnia(daphnia, velocities, turn_rates, fps_coef):
    ...
    v += fluct
    v *= (1 + random.choice(acceleration) * (1 + 20 * LIGHT_DISTRIBUTION[int(daphnia[0][0])][int(daphnia[0][1])]) *
          daphnia[4])
    v *= fps_coef
    # daphnia[5] = v
    # frame_TRANSITION()
    ...
    """if daphnia[3] == 1  and LIGHT_ON == 1 and daphnia[4] == 1:
        ...
        daphnia[-1] = random.choice(downAngles) + random.choice(turn_rates)*180/math.pi * fps_coef
        daphnia[6] = 1"""
    daphnia[5] = fps_coef * (1 + LIGHT_DISTRIBUTION[int(daphnia[0][0])][int(daphnia[0][1])])
